...
A `Graph` object may be used to describe a variety of processed data. Typically, a data payload will provide values along an axis. The axes are referred to within this object as the $x$ and $y$ axes, which respectively represent the dependent and independent variables. For example, the data may include samples of a discrete Fourier transform, and the corresponding frequency values may be specified as the $x$ axis using the `Graph` object.

The ordering of values provided in the data payload correspond to the axes described in the `Graph` object. For example, if the `Graph` objects provides an `x_start`, `x_stop`, and `x_step`,
the first value in the associated data represents the $y$ value at `x_start` and the next value represents the $y$ value at `x_start + x_step`. If `x_axis` is used instead, its length MUST be equal to the length of the corresponding data. See [below](#021-specifying-axes) for details about recording $x$ or $y$ axes.
